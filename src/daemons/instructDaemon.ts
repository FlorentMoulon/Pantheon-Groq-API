...
class InstructDaemon {
  ...
  async handleInstruction(pastIdeas: Idea[], instruction: string, apiType:number, openaiKey: string, openaiOrgId: string, instructModel: string) {
    ...
    return await CallChatModel(this.config.systemPrompt, userPrompt, apiType, openaiKey, openaiOrgId, instructModel);
  }
}
